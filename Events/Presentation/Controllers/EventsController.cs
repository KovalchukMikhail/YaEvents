    ...
    public class EventsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetEvents(CancellationToken token, [FromQuery] string? title = null, [FromQuery] DateTime? from = null, [FromQuery] DateTime? to = null, int page = 1, int pageSize = 10)
        {
            ...
            if (ModelState.ErrorCount > 0)
                throw new Presentation.Exceptions.ValidationException("В запросе на получение событий переданы некорректные параметры.") { ModelState = ModelState };
        ...
        public async Task<IActionResult> GetEvent(Guid id, CancellationToken token)
        {
            ...
            if (eventDto != null)
            {
                if (eventDto.Status == EventStatus.Existing)
                    ...
                    throw new Presentation.Exceptions.ValidationException("Запрашиваемый объект события помечен как удаленный") { EntityId = id };
            }
            else
        ...
        public async Task<IActionResult> PostEvent([FromBody] CreateEvent createEvent, CancellationToken token)
        {
            if (!CompareEventDates(createEvent.StartAt, createEvent.EndAt))
            {
                ...
                throw new Presentation.Exceptions.ValidationException("В запросе на добавление нового события переданы некорректные параметры.") { ModelState = ModelState };
            }
        ...
        public async Task<IActionResult> PutEvent(Guid id, [FromBody] CreateEvent createEvent, CancellationToken token)
        {
            if (!CompareEventDates(createEvent.StartAt, createEvent.EndAt))
            {
                ...
                throw new Presentation.Exceptions.ValidationException("В запросе на редактирование события переданы некорректные параметры.") { ModelState = ModelState, EntityId = id };
            }
        ...
        public async Task<IActionResult> DeleteEvent(Guid id, CancellationToken token)
        {
            ...
            if (requiredEvent != null)
            {
                if (await _eventService.DeleteEvent(id, token: token))
                    ...
                    throw new Presentation.Exceptions.ValidationException("Объект события уже был отмечен как удаленный") { EntityId = id };
            }
            else
