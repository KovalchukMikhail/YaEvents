        ...
        public async Task ProcessBookingAsync(Guid bookingId, CancellationToken stoppingToken = default)
        {
            try
            {
                ...
                if (@event == null)
                {
                    ...
                    throw new DomainValidationException("Не удалось обработать объект бронирования так как объект события отсутствует") { EntityId = booking.Id };
                }
                else if (@event.Status == EventStatus.Removed)
                {
                    await RejectBookingAsync(booking, @event, eventRepository, bookingRepository, stoppingToken);
                    throw new DomainValidationException("Не удалось обработать объект бронирования так как объект события помечен как удаленный") { EntityId = booking.Id };
                }
