...
namespace Infrastructure.Repositories.BookingsRepository
{
    public class BookingsRepository : IBookingsRepository
    {
        ...
        {
            return await _appDbContext.Bookings.Where(b => b.Id == id).FirstOrDefaultAsync(token);
        }
        ...
        public async Task<Booking[]> GetBookings(Guid userId, Guid eventId, CancellationToken token = default)
        {
            return await _appDbContext.Bookings.Where(b => b.UserId == userId && b.EventId == eventId).ToArrayAsync();
        }
    }
}
