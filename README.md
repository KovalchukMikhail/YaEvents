...
## Архитектура проекта
Проект состоит из 3 отдельных сервисов (Users, Events, Bookings) и библиотеке KafkaConfigurations содержащий разделяемый контракт и наименование топиков.
Каждый сервис разделен на слои Domain, Application, Infrastructure и Presentation.
Слой Domain содержит модели данных, исключения и перечисления и не зависит от остальных слоев.
Слой Application содержит бизнес логику и зависит только от слоя Domain. Взаимодействие с внешними сервисами и базой данных осуществляется по средствам интерфейсов. Слой Application не знает детали хранения данных или работы сервисов.
Слой Infrastructure отвечает за работу с базой данных и взаимодействие с внешними сервисами, он зависит от слоев Domain и Application.
Слой Presentation зависит от слоев Domain, Application и Infrastructure. В слое Presentation происходит инъекция зависимостей и реализована логика обработки запросов пользователя.
Слой Presentation содержит точку входа.

## Cхема потока данных BookingConfirmed
1. Сервис **Bookings**
- Пользователь создает бронирование со статусом **Pending**.
- Бронирование сохраняется в базу данных.
- Фоновый сервис обрабатывает бронирования со статусом **Pending**. Меняет статус на **Confirmed**.
Сохраняет бронирование в базу данных и с помощью **IBookingsProducer** публику **BookingConfirmed** в **Kafka**
2. Сервис **Events**
- В фоновом сервисе с помощью **IEventsConsumer** читает топик **booking-confirmed**
- Обрабатываются все новые сообщения и в случае корректных данных и наличия свободных мест происходит уменьшение свободных мест для события. Некорректные запросы логируются.

## Запуск сервиса
**Запуск из командной строки Windows**
1. Из папки проекта содержащий файл **docker-compose.yml** выполнить команду **docker compose up -d\** и дождитесь окончания выполнения команды.
2. Для запуска swagger каждого сервиса в адресной строке браузера ввести:
  - для Users **http://localhost:5258/swagger/index.html**
  - для Events **http://localhost:5172/swagger/index.html**
  - для bookings **http://localhost:5207/swagger/index.html**

## Авторизация в Swagger
1. Пользователи создаются в **Users** POST /auth/register
	Для создания пользователя с ролью администратора укажите **"role": "Admin"**
	Для создания обычного пользователя укажите **"role": "User"**
2. Авторизоваться используя POST /auth/login
3. После успешной авторизации скопировать из тела ответа токен.
4. Для авторизации в Events и Bookings нажмите Authorize. В появившемся окне вставить в поле Value токен и нажать Authorize. 

## Описание стратегии кэширования
Реализован паттерн Cashe-Aside для двух сценариев:
   Получение события по идентификатору;
   Топ-10 самых популярных событий.
Для обоих сценариев подобраны значения TTL. Выбор TTL обусловлен 2 факторами:
	1. Частота изменения данных.
	2. Рисками связанными с передачей устаревших данных.
Для топ-10 выбран значительный TTL (60 минут) так как частота изменений данных небольшая, а риски передачи устаревших данных незначительны.
Для отдельного события оба фактора существенны поэтому выбран короткий TTL (1 минута).
Для отдельного события реализован подход инвалидации кэша при изменении и удалении события. Так же при успешной обработки сообщения booking-confirmed от Kafka в связи с изменением количества свободных мест в объекте Event реализована инвалидация кэш.
Основания для выбора стратегии инвалидации:
	1. Лучшая поддерживаемость сервиса по сравнению со стратегией обновление при записи, в случае изменения модели данных события не потребуется вносить изменения в код кэширования.
...
7. Система наблюдаемости запускается вместе с остальными сервисами см. раздел **Запуск сервиса**

## Изменения в рамках одинадцатого спринта
1. Добавлены системы наблюдения Prometheus, Jaeger и Grafana.
2. В docker-compose.yml добавлены сервисы prometheus, jaeger, grafana
3. В репозиторий добавлены файлы prometheus.yml и dashboard.json

## Изменения в рамках десятого спринта
1. Добавлено кэширование данных c для сценариев:
   Получение события по идентификатору.
   Топ-10 самых популярных событий.
2. Кэширование реализовано с помощью Redis.
3. Интерфейс для работы с КЭШ определен в слое Application сервиса Events. Реализация интерфейса в слое Infrastructure.
4. При недоступности Redis ошибка логируется, но н пробрасывается пользователю.
5. При изменении или удалении события ключ события удаляется из КЭШ.
6. Контейнер Redis добавлен в docker-compose.yml.
7. Реализованы тесты для проверки сценариев:
   при попадании в кеш репозиторий не вызывается;
   при промахе данные берутся из репозитория и сохраняются в кеш;
   при мутирующих операциях кеш инвалидируется.

## Изменения в рамках девятого спринта
1. Монолит разделен на три самостоятельных сервиса, каждый со своей базой данных.
2. Сервисы Events и Bookings общаются сообщениями формат сообщения и имя топика вынесены в отдельную библиотеку YaEventsConfigurations.
...
5. В Users реализована регистрация пользователей и выдача токенов. В остальных сервисах реализована авторизация по токену полученному в Users.
6. Управление событиями доступно только роли Admin (403 для остальных), эндпоинты броней требуют аутентификации (401 без токена).
7. Все сервисы запускаются командой docker compose up -d из корневой папки приложения.
